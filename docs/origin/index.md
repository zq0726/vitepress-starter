...
2. `base`：当网站部署在 `GitHub Pages` 或 `Gitee Pages` 时会存在子路径，例如：`https://username.github.io/qing-vitepress/` ，需要设置 `base` 为 `/qing-vitepress/`;
...
const getHead = () => {
  return [["link", { rel: "icon", href: withBase("/logo.svg") }]];
};
```
...
```json
{
  ...
  "selectors_exclude": ["outline-link"]
}
```
